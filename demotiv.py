...
class DemotivMod(loader.Module):
    """Demotivation with @super_rjaka_demotivator_bot"""

    strings = {'name': 'Demotiv'}
    ...
    async def dmtcmd(self, event):
        ...
        await event.edit('<code>Wait..</code>')
        async with event.client.conversation(chat) as conv:
            try:
                response = conv.wait_event(events.NewMessage(incoming=True,
                                                             from_users=1016409811))
                if not self_mess:
                ...
                await event.reply('<code>Unblock </code> @super_rjaka_demotivator_bot')
                return
            await event.delete()
            if reply_and_text:
