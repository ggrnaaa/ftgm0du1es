...
class PrikolMod(loader.Module):
    ...
    async def intcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media2(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem2(pic)
    ...
    async def smcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media3(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem3(pic)
    ...
    async def moskcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media4(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem4(pic)
    ...
    async def aktcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media5(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem5(pic)
    ...
    async def zpmcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media6(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem6(pic)
    ...
    async def zbtcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media7(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem7(pic)
    ...
    async def drcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media8(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem8(pic)
    ...
    async def zapcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media9(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem9(pic)
    ...
    async def pecmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media11(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem11(pic)
    ...
    async def rkcmd(self, message):
        ...
        if not reply:
            ...
            pic = await check_media12(message, reply)
            if not pic:
                await utils.answer(message, 'это не картиночка')
                return
            what = mem12(pic)
...
def lol1(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
def lol2(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
def lol3(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
def lol4(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
def lol5(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
...
def lol6(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
def lol7(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
def lol8(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 1, size * 2))
    background.paste(overlay, cords)


def mem8(image):
    pics = requests.get("https://raw.githubusercontent.com/ggrnaaa/ftgm0du1es/master/%D0%BA%D0%B0%D1%80%D1%82%D0%B8%D0%BD%D0%BA%D0%B82/IMG_20200920_165911_347_1600610383875.jpg" )
    pics.raw.decode_content = True
    img = Image.open(io.BytesIO(pics.content)).convert("RGB")
...
def lol9(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
async def check_media9(message, reply):
    if reply and reply.media:
        if reply.photo:
            data = reply.photo
        elif reply.document:
...
def lol11(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
async def check_media11(message, reply):
    if reply and reply.media:
        if reply.photo:
            data = reply.photo
        elif reply.document:
...
def lol12(background, image, cords, size):
    overlay = Image.open(BytesIO(image))
    overlay = overlay.resize((size * 2, size * 1))
    background.paste(overlay, cords)
...
async def check_media12(message, reply):
    if reply and reply.media:
        if reply.photo:
            data = reply.photo
        elif reply.document:
